...
Set_Memory(){
	...
	echo && echo ${Separator_1} && echo -e "	核心数 : ${Green_font_prefix}${Memory}${Font_color_suffix}" && echo ${Separator_1} && echo
}
...
Apply_Config(){
  ...
  sed -i 's/l2wQLimit="256 2048"/l2wQLimit="'${Memory}'"/' /appex/etc/config
  sed -i 's/w2lQLimit="256 2048"/w2lQLimit="'${Memory}'"/' /appex/etc/config
